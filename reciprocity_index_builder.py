...
function sortRows(rows) {{
  ...
  return [...rows].sort((a, b) => {{
    ...
    if (mode === 'extra-steps') {{
      const d = Number(a.dataset.extraSteps) - Number(b.dataset.extraSteps);
      if (d !== 0) return d;
...
if (params.get('sort') && {allowed_sort_values}.includes(params.get('sort'))) sortSelect.value = params.get('sort');
